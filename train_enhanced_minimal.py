...
def main():
    parser = argparse.ArgumentParser(
        description="Train with minimal compression (10% target)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="enhanced_model_minimal",
    ...
    parser.add_argument(
        "--epochs",
        type=int,
        default=5,  # Increased for better learning
    ...
    parser.add_argument(
        "--log_steps",
        type=int,
        default=5,  # More frequent logging
    ...
    # Determine compression layer indices - use only ONE layer at the end
    compression_indices = [11]  # Single compression layer at the end of the model

    # Create compressible model with EXTREMELY conservative initial thresholds
    print(
        "Creating compressible model with MINIMAL compression target (10% improvement)..."
    )
    model = CompressibleLanguageModel(
        base_model_name=args.model_name,
        compression_layer_indices=compression_indices,
        # Extremely conservative threshold - almost no compression
        similarity_threshold=0.995,  # Ultra-high threshold to prevent over-compression
        use_residuals=True,  # Enable residuals for better reconstruction
        percentile_threshold=99.0,  # Only compress the most similar tokens
    ...
    # Enhanced validation function with more detailed quality checks
    def validation_callback(model, step):
        """Comprehensive quality check during training"""
        ...
        target_ratio = 1.1  # Target 10% improvement

        # If compression is too high OR quality is too low, reduce compression
        if compression_ratio > target_ratio * 1.2 or avg_quality < 0.5:
            print("⚠️ WARNING: Compression too aggressive or quality too low!")
            for layer in model.compression_layers:
                # Make threshold even higher (less compression)
                layer.base_threshold = min(0.999, layer.base_threshold + 0.001)
    ...
    print("Starting training with MINIMAL compression target...")
    train(
        model=model,
    ...
    print("\nGenerating sample text with minimal compression model:")
    prompts = [
        "Artificial intelligence will",
        "The future of technology is",
